...
struct _device_data{
    struct cdev  chardev;  //字符设备对应的数据结构，由linux内核定义
    unsigned char *buffer;
    int npos;    //设备字符数最大值
}*mydata[MAX_DEVICES];

static ssize_t device_read(struct file *filp, char * buff, size_t len, loff_t* offset) //len要读取长度、offset文件偏移量（对文件进行操作的起始位置）
{
    int nlen=len;
    struct _device_data* pdb=filp->private_data;
    if(nlen>pdb->npos-*offset)
        nlen=pdb->npos-*offset;   //如果文件中剩余字符数不足，就取到最大长度
...
static ssize_t device_write(struct file *filp, const char * buff, size_t len, loff_t* offset)
{
    ...
    if(nlen>BUFFER_SIZE-pdb->npos)
        nlen=BUFFER_SIZE-pdb->npos;     //如果装不下设备中的字数，能装多少装多少
    if(nlen==0)
        return -ENOMEM;
...
static int device_open(struct inode *inode, struct file * filp)
{
    ...
    if(!mydata[nminor]->buffer)
        mydata[nminor]->buffer=(unsigned char *)vmalloc(BUFFER_SIZE);  //设备没有buffer就创建一个
    if(!mydata[nminor]->buffer)    //创建失败
        return -ENOMEM;
